...
- `discord.py` bot with slash commands, a direct-mention/reply public ask listener, and an auto-scan gate for high-confidence local answer opportunities, soft-warning candidates, and explicit bot-topic banter opportunities.
- Message Content intent enabled so `@ChaosX <question>` and replies to stored ChaosX answers can reuse `/ask`, Hoops can use owner/admin mode by mentioning or replying to ChaosX, owner-requested `/admin ask` can analyze message bodies, and auto-scan can classify new allowed-guild messages locally before routing any public response through the configured public model.
...
- Auto-scan uses a local deterministic gate: it ignores bots/webhooks, DMs, unauthorized guilds, slash-like text, configured excluded channels, direct mentions, and replies already handled by the ask/admin path. The gate only decides whether to engage and supplies source/context metadata; every public auto-scan answer, bot-topic banter reply, or soft warning is generated by the configured public model. Successful auto Q&A is saved into the Q&A table, seeds reply-chain memory for replies to those auto-answers, and records acted scan events. Soft warnings also post a notice to the automation channel.
...
- Message Content use must stay inside the approved surfaces: direct/reply asks, Hoops' owner/admin mention/reply path, owner-requested `/admin ask` message analysis, and the auto-scan gate. Auto-scan must stay silent unless the local gate finds a 100-confidence answer opportunity, explicit bot-topic banter opportunity, or obvious soft-warning candidate; any public text from that point must be model-generated, not hardcoded.
