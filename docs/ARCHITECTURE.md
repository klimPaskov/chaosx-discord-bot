...
- `discord.py` bot with slash commands, a direct-mention/reply public ask listener, and a zero-token auto-scan listener for high-confidence local answers, soft warnings, and explicit bot-topic banter.
...
- Message Content use must stay inside the approved surfaces: direct/reply asks, Hoops' owner/admin mention/reply path, owner-requested `/admin ask` message analysis, and zero-token auto-scan. Auto-scan must remain deterministic/no-Hermes-per-message and must stay silent unless it has a 100-confidence local answer, an explicit bot-topic banter match, or an obvious soft-warning match.
