...
- Provides:
  ...
  - Auto-scan automation checks each new allowed-guild message with local rules only. It auto-answers exact event/scenario/cluster/status/testing/server-help questions, saves successful auto Q&A into `/admin qna`, seeds reply-chain memory for replies to those auto-answers, joins explicit conversations about ChaosX/the bot with short deterministic banter, gives soft warnings for obvious rule problems, and posts warning notices to `CHAOSX_AUTO_SCAN_NOTIFY_CHANNEL_ID` or `CHAOSX_AUTOMATION_REMINDER_CHANNEL_ID`.
