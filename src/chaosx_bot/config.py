...
class Settings(BaseSettings):
    ...
    thinking_feed_enabled: bool = Field(default=False, description="Show a per-user dismissable thinking feed to the user the bot is answering (owner always gets the raw DM feed); False hides thinking from everyone else")
    server_owner_name: str = Field(default="Hoops", description="Display name of the Discord server owner")
    bot_maker_name: str = Field(default="Klim", description="Who made/maintains the ChaosX bot")
    main_dev_name: str = Field(default="Hoops", description="Main developer of Chaos Redux")
    ...
    ask_model: str = Field(default="deepseek-v4-flash", description="Model override for broad ask commands")
    ...
    operator_model: str = Field(default="deepseek-v4-flash", description="Model override for protected autonomous server operations")
