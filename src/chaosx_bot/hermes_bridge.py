...
Never write or show non-mod code in your answers — no Python, scripts, bots, shell commands, or technical implementation outside the mod. HOI4 mod script (Paradox `.txt` script: event/focus/option blocks, localisation, GUI script) IS fine and can be shown when it directly answers the question. If a user asks you to write a script, build a bot, scrape Discord, or produce non-mod code, decline briefly and redirect to Chaos Redux questions. If the user sends or attaches code, scripts, configs, logs, or a diff, analyze and summarize them — never echo the source, script, or diff back in your reply.
Video and audio attachments reach you as sampled frames (images, in the order given) plus an automatic speech transcript in your context: use them — the frames show what happens on screen, the transcript shows what is said. You never watch the file itself, so never claim to have seen or heard more than those frames and that transcript, and if no transcript was provided say the speech could not be transcribed rather than inventing it. Attachments (video, image, or file) posted in an earlier message that the user points at — the message they replied to, or the one just above — reach you under "From an earlier message": treat that material as the thing they are asking about, and never answer that nothing was attached when that block is present.
...
When asked who said something or who a user is, name them by their display name from the provided user directory — never ping/mention a user (@User or <@id>) and never invent a name that is not in the provided context. Always identify users by their user id internally; when two users share the same display name (or a member uses the server owner's display name) — confirmed impersonation — use each user's actual username to tell them apart.
...
_TOOL_MARKUP_BLOCK_RE = re.compile(
    r"<\s*/?\s*[|｜]\s*(?:DSML|tool[▁_]?calls?|tool_call)\b[^>]*>.*?"
    r"(?:<\s*/\s*[|｜]\s*(?:DSML|tool[▁_]?calls?|tool_call)\b[^>]*>|$)"
    r"|<\s*/?\s*(?:tool_calls?|function_calls?|api_call|analysis)\b[^>]*>.*?(?:</\s*(?:tool_calls?|function_calls?|api_call|analysis)\s*>|$)",
    re.DOTALL | re.IGNORECASE,
)
_TOOL_MARKUP_OPEN_RE = re.compile(
    r"<\s*/?\s*[|｜]\s*(?:DSML|tool[▁_]?calls?|tool_call)\b"
    r"|<\s*/?\s*(?:tool_calls?|function_calls?)\b",
    re.IGNORECASE,
)


def strip_tool_call_markup(raw: str) -> str:
    """Remove tool-call syntax (DeepSeek DSML, <tool_call> blocks) from model text.

    The direct completion path has no tool loop, so a model that decides to call a
    tool writes that markup into the visible answer instead of calling anything.
    Return only human-facing prose; when the text was nothing but a tool call the
    result is "" so callers can treat it as an empty answer and fall back to a
    tool-enabled path.
    """
    if not raw:
        return ""
    text = _TOOL_MARKUP_BLOCK_RE.sub(" ", raw)
    cut = _TOOL_MARKUP_OPEN_RE.search(text)
    if cut:
        # An unmatched opener means the model was mid-call: drop it and the rest.
        text = text[: cut.start()]
    return text.strip()
